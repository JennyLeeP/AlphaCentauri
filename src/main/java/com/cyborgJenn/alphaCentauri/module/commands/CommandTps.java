...
public class CommandTps extends Command {
	...
	private double getTickMs(World world) {
		return getTickTimeSum(world == null ? AlphaCentauri.server.tickTimeArray : AlphaCentauri.server.worldTickTimes.get(world.provider.getDimension())) * 1.0E-6D;
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1) {
			...
			for (World world : AlphaCentauri.server.worldServers) {
				tps = getTps(world);
			...
			World world = AlphaCentauri.server.worldServerForDimension(dim);
