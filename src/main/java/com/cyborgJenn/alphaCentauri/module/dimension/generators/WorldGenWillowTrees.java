...
public class WorldGenWillowTrees extends WorldGenAbstractTree{
	private final int treeId;

	public WorldGenWillowTrees(boolean flag, int id)
	{
		super(flag);
		treeId=id;
	}
	...
	private void addBranch(int x, int y, int z, int x1, int z1, Random random, World world)
	{
		if (random.nextInt (3) == 0)
		{
			...
			for (int a = 0 ; a < length ; a++)
			{
				...
				//	world.setBlock (xCoord * a / length + x, yCoord * a / length + y, zCoord * a / length + z, ModBlocks.log1, treeId, 0x2);
			}
	...
	public boolean generate (World worldIn, Random rand, BlockPos position)
	{
			...
			for (int j2 = xCoord - byte0 ; j2 <= xCoord + byte0 && flag ; j2++)
			{
				for (int j3 = zCoord - byte0 ; j3 <= zCoord + byte0 && flag ; j3++)
				{
					if (i1 >= 0 && i1 < world.getHeight())
					{
						...
						if (i4.isAir(world, j2, i1, j3) || i4 == ModBlocks.leaves)
							continue;
						if (i4 == Blocks.water)
						{
							if (i1 > yCoord)
        ...
        boolean isSoil = block2.canSustainPlant(world, xCoord, yCoord - 1, zCoord, ForgeDirection.UP, (BlockSapling)ModBlocks.saplings);
       
        		
		if (!(isSoil || yCoord >= world.getHeight() - height - 1))
			return false;
		...
		for (int n = 0 ; n < 2 ; n++)
		{
			...
			for (int a = 0 ; a < length ; a++)
			{
				if(world.isAirBlock(x2 * a / length + x, y2 * a / length + y, z2 * a / length + z))
					world.setBlock (x2 * a / length + x, y2 * a / length + y, z2 * a / length + z, ModBlocks.log1, treeId, 0x2);
		...
		{
			world.setBlock(xCoord, yCoord + l1, zCoord, ModBlocks.log1, treeId, 0x2);
		}
*/
		return true;
	}
