...
public class MapGenACRavine extends MapGenRavine {
	...
	private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? state.getBlock() == ModBlocks.ACGRASS : state.getBlock() == biome.topBlock);
    }
	@Override
	protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
		net.minecraft.world.biome.Biome biome = world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
        IBlockState state = data.getBlockState(x, y, z);
        IBlockState top = isExceptionBiome(biome) ? ModBlocks.ACGRASS.getDefaultState() : biome.topBlock;
        IBlockState filler = isExceptionBiome(biome) ? ModBlocks.ACDIRT.getDefaultState() : biome.fillerBlock;

        if (state.getBlock() == ModBlocks.ACSTONE || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock())
        {
            if (y - 1 < 10)
            {
