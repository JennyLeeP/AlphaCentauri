...
public class WorldGenACClay extends WorldGenerator {

	private final Block clayblock = Blocks.CLAY;
	private final int quantity;
	public WorldGenACClay(int quantity) {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        {
            int i = rand.nextInt(this.quantity - 2) + 2;
                ...
                for (int l = position.getZ() - i; l <= position.getZ() + i; ++l)
                {
                    ...
                    if (i1 * i1 + j1 * j1 <= i * i)
                    {
                        for (int k1 = position.getY() - 1; k1 <= position.getY() + 1; ++k1)
                        {
                            ...
                            if (block == ModBlocks.ACDIRT || block == Blocks.CLAY)
                            {
