...
public class ACTeleporter extends Teleporter 
{
			...
			for (int l = -2; l <= 2; ++l){
				for (int i1 = -2; i1 <= 2; ++i1){
					for (int j1 = -1; j1 < 3; ++j1){
						...
						int i2 = (k + (i1 * b1)) - (l * b0);
						boolean flag = j1 < 0;
						worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? ModBlocks.gateAlphaCentauri.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
            ...
            for (int i1 = -128; i1 <= 128; ++i1)
            {
                ...
                for (int j1 = -128; j1 <= 128; ++j1)
                {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.worldServerInstance.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2)
                    {
						...
						if (this.worldServerInstance.getBlockState(blockpos1).getBlock() == ModBlocks.alphaCentauriPortal.getDefaultState()){
							for (blockpos2 = blockpos1.down(); this.worldServerInstance.getBlockState(blockpos2).getBlock() == ModBlocks.alphaCentauriPortal; blockpos2 = blockpos2.down())
                            {
                                blockpos1 = blockpos2;
                            }
            ...
            for (int j7 = -1; j7 <= 1; ++j7)
            {
                for (int l7 = 1; l7 < 3; ++l7)
                {
                    for (int k8 = -1; k8 < 3; ++k8)
                    {
                        ...
                        boolean flag = k8 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k9, k10, k11), flag ? ModBlocks.gateAlphaCentauri.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = ModBlocks.alphaCentauriPortal.getDefaultState().withProperty(BlockAlphaCentauriPortal.AXIS, l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
        ...
        for (int i8 = 0; i8 < 4; ++i8)
        {
            for (int l8 = 0; l8 < 4; ++l8)
            {
                for (int l9 = -1; l9 < 4; ++l9)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(l10, l11, k12), flag1 ? ModBlocks.gateAlphaCentauri.getDefaultState() : iblockstate, 2);
                }
            }
	...
	public class AlphaCentauriPortalPosition extends BlockPos
    {
        /** The worldtime at which this PortalPosition was last verified */
        public long lastUpdateTime;

        public AlphaCentauriPortalPosition(BlockPos pos, long lastUpdate)
        {
