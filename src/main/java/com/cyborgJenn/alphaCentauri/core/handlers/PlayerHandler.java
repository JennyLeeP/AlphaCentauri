		...
		if (player != null && !player.worldObj.isRemote)
		{
			try {
				...
				if (file1 == null || !file1.exists() || data == null
						|| data.hasNoTags())
				{
					AlphaCentauri.logger.warn("Accesssory Data not found for " + player.getDisplayNameString() + ". Trying to load backup data.");
			...
			{
				AlphaCentauri.logger.fatal("Error loading accessory inventory");
		...
		if (player != null && !player.worldObj.isRemote)
		{
			try {
				if (file1 != null && file1.exists())
				{
					...
					{
						AlphaCentauri.logger.error("Could not backup accessory file for player " + player.getDisplayNameString());
					}
				}
				...
				{
					AlphaCentauri.logger.error("Could not save accessory file for player " + player.getDisplayNameString());
					e.printStackTrace();
					if (file1.exists())
					{
						try {
			...
			{
				AlphaCentauri.logger.fatal("Error saving Accessory inventory");
				exception1.printStackTrace();
			}
		}
	}
